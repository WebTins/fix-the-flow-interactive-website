# Buurtcampuskrant

Tijdens sprint 5 ben ik vooral bezig geweest met het uitbreiden van de website. Ik heb me ook gericht om een interactie toe te voegen voor een filter omdat het ook de bedoeling is dat de gebruikers/doelgroepen naar hun eigen doelgroep kunnen filteren. Verder heb ik ook de focus gelegd op de toegankelijkheid van de website omdat er veel kleuren worden gebruikt die overeen moeten komen met de kleurcontrast. Hierbij heb ik ervoor gezorgd dat alle Lees meer knoppen dezelfde stijling krijgen met dezelfde kleur. Hierdoor wordt het meer leesbaar voor de gebruiker en geeft het ook een mooie uiterlijk aan de website.

### Wat is de Buurtcampuskrant?
De Buurtcampuskrant is een digitale krant die iedereen kan lezen maar voornamelijk voor mensen die in Amsterdam wonen. Het doel is om iedereen op de hoogte te brengen wat er gebeurd in de verschillende buurten. De doelgroepen van de Buurtcampus zijn: Studenten, Bewoners, Wijkpartners en HvA medewerkers. De Buurtcampuskrant brengt dus mensen samen die ook meer te komen weten over andere buurten als zij er geïnteresseerd in zijn.

### Vraag van de opdrachtgever
Maak een website die gebruikers kunnen gebruiken als een digitale krant waar ze nieuws kunnen lezen en kunnen filteren op doelgroep. De digitale krant moet dezelfde uiterlijk en gevoel geven zoals de fysieke krant die elke 1,5 jaar wordt uitgebracht.

Link naar de website: https://webtins.github.io/fix-the-flow-interactive-website/

## Inhoudsopgave

  * [Beschrijving](#beschrijving)
  * [Kenmerken](#kenmerken)
  * [Bronnen](#bronnen)
 
## Beschrijving

### Ontwerpkeuze
Het ontwerp dat ik heb gekozen is de afgelopen paar sprints wel aangepast maar ook weer hetzelfde gebleven. De reden waarom ik mijn onterwerpkeuzes heb aangepast is omdat ik bij een toegankelijkheidstest heb ontdekt dat de het contrast van de knoppen en het achtergrond niet voldoende waren. Dus ik heb besloten om de knoppen een ander kleur te geven die het contrast verbeteren. De reden waarom het contrast belangrijk is, is omdat gebruikers met visuele beperkingen ook de knoppen goed moeten zien om te kunnen interacteren. 

<img width="1373" height="811" alt="image" src="https://github.com/user-attachments/assets/15e197e3-4431-4300-9391-739b19d5d5c5" />


<img width="677" height="789" alt="image" src="https://github.com/user-attachments/assets/8043eba7-dd51-4858-8b8a-309cccb3fb04" />



* Kleur verandering van Lees meer knop

  <img width="158" height="57" alt="image" src="https://github.com/user-attachments/assets/3b0ea827-e90a-4058-a26b-6a6c20093a2f" /> <img width="162" height="58" alt="image" src="https://github.com/user-attachments/assets/4d3ea51a-9460-4f8c-9c1c-ee7f847841bf" />

### Pagina's

In deze sprint heb ik 3 pagina's gemaakt van de locaties: Zuidoost, Oost en Nieuw-West

* Link naar Zuidoost: https://webtins.github.io/fix-the-flow-interactive-website/pages/zuidoost.html

* Link naar Oost: https://webtins.github.io/fix-the-flow-interactive-website/pages/oost.html

* Link naar Nieuw-West: https://webtins.github.io/fix-the-flow-interactive-website/pages/nieuw-west.html

### Feature

**Scroll animatie** 

De scroll animatie heb ik toegevoegd voor een mooie verschijnsel van de artikelen waneer je gaat scrollen. Dit geeft een de website eerst een rustige uiterlijk zodat je niet eerst alle artikelen krijgt te zien.

Feedback:

> De artikelen komen vervagend tervoorchijn wanneer je gaat scrollen tot het volledige artikel.

Feedforward:

> De artikelen verwdijnen vervagend wanneer de artikelen niet volledig in beeld zijn

https://github.com/user-attachments/assets/7d89cd5b-175e-45ee-ba13-21968aaeea4d

**Interactie voor filteren**

Feedback:

> De filterknop komt tevoorschijn met 4 doelgroepen en een gereed knop

> Wanneer je op de gereed knop klikt, dan gaat de filter weg

Feedforward: 

> De filterknop krijgt een ander kleur wanneer je je muis erover heen houdt

https://github.com/user-attachments/assets/87d70931-2b3b-4ece-970d-dc87af0a38a2

* Invoerveld voor emailadres

<img width="643" height="98" alt="image" src="https://github.com/user-attachments/assets/0b9c4d15-4b01-403f-8d3d-b7d915af0675" />

* Foutmelding bij geen invoer

<img width="524" height="105" alt="image" src="https://github.com/user-attachments/assets/96b6490d-9357-48d9-848c-dbb90a49743c" />


## Kenmerken

### Toegankelijkheid in HTML

De website voldoet aan het contrast via de toegankelijkheidstesten:

<img width="1919" height="909" alt="image" src="https://github.com/user-attachments/assets/66cee97d-11f4-4c3f-bc20-287e0291392d" />

<img width="1919" height="905" alt="image" src="https://github.com/user-attachments/assets/d2907dc0-377a-490a-962a-ba146c576562" />

<img width="1919" height="906" alt="image" src="https://github.com/user-attachments/assets/cae44ff4-7399-47be-9973-1bb6c2ff988d" />

<img width="1919" height="909" alt="image" src="https://github.com/user-attachments/assets/3191bfa3-8d37-438e-92d6-eb5457005afb" />

### Belangrijke aspecten in CSS

De website maakt gebruikt van de stylesheet die wordt ingeladen waar de huisstijl van het Buurtcampus in staat. Verder is er ook een aparte CSS bestand die andere stijling toevoegd in de website.

### Belangrijke aspecten Javascript

Javascript wordt gebruikt om de popup van de filter te laten verschijnen en ook om het weer te laten verdwijnen.

## Bronnen

Figma: https://www.figma.com/design/GRdLpkhxYTfgDDJRlVFulP/Untitled?node-id=0-1&t=Itwj2EkjxaMF04SN-1

Stylesheet: https://webtins.github.io/look-and-feel-styleguide/

## Licentie

This project is licensed under the terms of the [MIT license](./LICENSE).

